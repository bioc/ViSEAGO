...
```{r grViz,echo=F,eval=T,fig.height=5.5} 
###################
...
Firtsly, install the `r BiocStyle::Biocpkg("ViSEAGO")` R package from [Bioconductor](https://www.bioconductor.org/), or `r Githubpkg("UMR_BOA","ViSEAGO")` from [github](https://github.com/UMR-BOA-project/ViSEAGO) repository.

```{r ViSEAGO_install}
###################
# load ViSEAGO package from Bioconductor
BiocManager::install("ViSEAGO", version = "3.8")

###################
# load ViSEAGO package from github
remotes::install_github("UMR-BOA-project/ViSEAGO",build_opts = c("--no-resave-data","--no-manual"))
```

You can also optionally create directories to store gene lists in the input folder, store all the intermediate R objects in the cache folder, and export the results in the output folder.

```{r ViSEAGO_directories}
###################
# create data directories (input, cache,output)
base::dir.create("./data/input",recursive=T,showWarnings=F)
base::dir.create("./data/output",showWarnings=F)
base::dir.create("./data/cache",showWarnings=F)
```

# Genes of interest

```{r grViz_genes,echo=F,eval=T,fig.height=4} 
###################
# print diagram with focus one the gene list
ViSEAGO::pkgdiagram(x=c("black","white","white","white","white"))
```

Firstly, you need to provide the gene *background* (= expressed genes) and *selection* in the created `./data/input/` subfolder, before reading the files.

```{r geneList_input}
###################
# load genes background
background<-base::scan("./data/input/background.txt",quiet=T,what="")

###################
# load gene selection
selection<-base::scan("./data/input/selection.txt",quiet=T,what="")
```

# GO annotation of genes

```{r grViz_annotations,echo=F,eval=T,fig.height=4} 
###################
...
This method is available using the `ViSEAGO::annotate(id,EntrezGene,ortholog = T)`.
...
myGENE2GO<-ViSEAGO::annotate("bioconductor_id",Bioconductor)

###################
# load GO annotations from EntrezGene
myGENE2GO<-ViSEAGO::annotate("EntrezGene_id",EntrezGene)

###################
# load GO annotations from EntrezGene
# with the add of GO annotations from orthologs genes (see above)
myGENE2GO<-ViSEAGO::annotate("EntrezGene_id",EntrezGene,ortholog = T)

###################
# load GO annotations from Ensembl
myGENE2GO<-ViSEAGO::annotate("Ensembl_id",Ensembl)
...
```{r grViz_enrichment,echo=F,eval=T,fig.height=4} 
###################
...
ViSEAGO::show_table(BP_sResults,"./data/output/BP_sResults.xls")
```
...
ViSEAGO::Upset(BP_sResults,file="./data/output/OLexport.xls")
```

# GO terms Semantic Similarity

```{r grViz_SS,echo=F,eval=T,fig.height=4} 
###################
...
```{r grViz_vis,echo=F,eval=T,fig.height=4} 
###################
...
###################           
# print MDSplot
ViSEAGO::MDSplot(myGOs,file="./data/output/mdsplot1.png")
```
...
In step 2, all previously unassigned objects are tested for sufficient proximity to clusters detected in the first step using a modified Partitioning Around Medoids (PAM). If the closest cluster is close enough in a precise sense that we define below, the object is assigned to that cluster. The PAM stage will be performed using *pamStage=T* parameter, and respect the dendrogram using *pamRespectsDendro=T* parameter, in the sense an object can be PAM-assigned only to clusters that lie below it on the branch that the object is merged into.
...
ViSEAGO::show_heatmap(
  ...
  "GOterms",
  "./data/output/cluster_heatmap_Wang_wardD2.png"
)
```
...
ViSEAGO::show_table(
  Wang_clusters_wardD2,
  "./data/output/cluster_heatmap_Wang_wardD2.xls"
)
```
...
ViSEAGO::show_heatmap(Wang_clusters_wardD2,"GOclusters",
"./data/output/Wang_clusters_wardD2_heatmap_groups.png")
```
